...
- **Parité CLI :** ce use case recoupe `ModelService.AddFull` déjà exposé côté REST (`POST /api/components`) — l'équivalent CLI est `myr model add`, à étendre vers `AddFull` pour exposer `Category`, `ParentID`, `LicenseID` (voir `specs/3-Conception/DC_CLI_Model.md` § 3.1, § 6 point 4), comme pour UCCE01/03. Le plugin CAO constitue lui-même un canal non-GUI équivalent en esprit à un script CLI : il appelle l'API REST de façon programmatique — le principe de parité CLI/REST (CLAUDE.md) est donc respecté par construction dès que `myr model add` sera étendu ; aucune lacune de conception domaine ici, contrairement aux autres UC de ce lot.
